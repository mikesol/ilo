...
describe("kitchen sink: fold extensions + negative", () => {
  ...
  it("scoped lambda called twice — no stale cache", async () => {
    ...
    interp["test/apply-twice"] = async function* (e) {
      ...
      const paramId = "param";
      const r1: unknown = yield {
        type: "recurse_scoped",
        child: bodyId,
        bindings: [{ paramId, value: 3 }],
      } as unknown as number;
      const r2: unknown = yield {
        type: "recurse_scoped",
        child: bodyId,
  ...
  it("NEGATIVE: missing handler for node kind", async () => {
    ...
    const { interp } = createTestInterp();
    await expect(koan.fold("root", adj, interp)).rejects.toThrow("no handler");
  });
  ...
  it("NEGATIVE: missing node in adjacency", async () => {
    ...
    const { interp } = createTestInterp();
    await expect(koan.fold("root", adj, interp)).rejects.toThrow();
  });
  ...
  it("NEGATIVE: commit rejects dangling child reference", () => {
    ...
    const mutated = koan.removeEntry(d, childToRemove);
    expect(() => koan.commit(mutated as any)).toThrow();
  });
  ...
  it("NEGATIVE: error/fail without try propagates to caller", async () => {
    ...
    const { interp } = createTestInterp();
    await expect(koan.fold("root", adj, interp)).rejects.toThrow("uncaught");
  });
  ...
  it("NEGATIVE: trait type mismatch at elaboration", () => {
    expect(() => koan.app(koan.eq(1, "hello") as any)).toThrow();
  });
});
