...
import {
  ...
  selectWhere, mapWhere, replaceWhere,
  byKind, byKindGlob, isLeaf, hasChildCount, not, and, or,
...
describe("Map & Replace", () => {
  test("mapWhere renames kind", () => {
    const m = mapWhere(p(), byKind("num/add"), (e) => ({
      kind: "num/sub" as const, children: e.children, out: e.out,
    }));
  ...
  test("replaceWhere + fold = correct", async () => {
    const r = replaceWhere(p(), byKind("num/add"), "num/sub");
    expect(await fold(r, defaults(stdPlugins))).toBe(-5);
  });
  ...
  test("mapWhere compound pred", () => {
    const m = mapWhere(p(), and(byKindGlob("num/"), isLeaf()), (e) => ({
      kind: "mapped/lit" as const, children: e.children, out: e.out,
    }));
...
describe("GC", () => {
  ...
  test("deeply unreachable removed", () => {
    const d = addEntry(
      addEntry(dirty(p()), "x", {
        kind: "dead" as const, children: ["y"] as const, out: undefined,
      }),
      "y", { kind: "dead" as const, children: [] as const, out: undefined },
    );
    const g = gc(d);
...
describe("Dirty/Commit", () => {
  ...
  test("addEntry", () => {
    const d = addEntry(dirty(p()), "f", {
      kind: "debug/log" as const, children: ["e"] as const, out: undefined,
    });
  ...
  test("swapEntry", () => {
    const d = swapEntry(dirty(p()), "c", {
      kind: "num/sub" as const, children: ["a", "b"] as const, out: 0,
    });
  ...
  test("swap→commit preserves other entries", () => {
    const c = commit(swapEntry(dirty(p()), "c", {
      kind: "num/sub" as const, children: ["a", "b"] as const, out: 0,
    }));
...
describe("Splice", () => {
  test("removes matched, reconnects children", () => {
    const s = spliceWhere(p(), byKind("num/add"));
    expect("c" in s.__adj).toBe(false);
    expect(s.__adj["e"].children).toEqual(["a", "b", "d"]);
  });
  test("wrap-then-splice round-trip", () => {
    const rt = spliceWhere(wrapByName(p(), "c", "debug/wrap"), byKind("debug/wrap"));
    expect(rt.__adj["e"].children).toEqual(["c", "d"]);
    expect("f" in rt.__adj).toBe(false);
  });
  test("splice leaf: parent children shrink", () => {
  ...
  test("double-wrap then splice", () => {
    ...
    expect(spliceWhere(w2, byKind("debug/wrap")).__adj["e"].children).toEqual(["c", "d"]);
  });
});
...
describe("Fold integration", () => {
  ...
  test("add→sub: (3-4)*5 = -5", async () => {
    expect(await fold(replaceWhere(p(), byKind("num/add"), "num/sub"), defaults(stdPlugins))).toBe(-5);
  });
  test("add→mul: (3*4)*5 = 60", async () => {
    expect(await fold(replaceWhere(p(), byKind("num/add"), "num/mul"), defaults(stdPlugins))).toBe(60);
  });
});
