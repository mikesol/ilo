...
/** Type-level normalize result placeholder for staged migration. */
export type AppResult<_Reg, _Expr> = NExpr<unknown, string, Record<string, RuntimeEntry>, string>;
...
function elaborate(
  ...
  function visitValue(value: unknown, expectedTag?: string): { id: string; outType: string } {
    ...
    if (value !== null && typeof value === "object") {
      if (Array.isArray(value)) {
        ...
        const id = alloc();
        entries[id] = { kind: "core/tuple", children: childIds, out: undefined };
      ...
      const id = alloc();
      entries[id] = { kind: "core/record", children: childMap, out: undefined };
      return { id, outType: "object" };
    }
  ...
  function visitExpr(node: CExpr<unknown>, expectedTag?: string): { id: string; outType: string } {
    ...
    if (kind === "core/access") {
      ...
      const id = alloc();
      entries[id] = { kind: "core/access", children: [parent.id], out: args[1] };
    ...
    if (kind in traitMap) {
      ...
      if (!resolved) {
        throw new Error(`No ${kind} implementation for type: ${left.outType}`);
      }
      const id = alloc();
...
export function createApp<const P extends readonly Plugin[]>(...plugins: P) {
  ...
  return function app<Expr extends CExpr<unknown>>(expr: Expr): AppResult<unknown, Expr> {
    return elaborate(expr, liftMap, traitMap, kindInputs, kindOutputs) as AppResult<unknown, Expr>;
  };
}
